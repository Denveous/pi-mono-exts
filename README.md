...
MCP client extension for [pi](https://github.com/badlogic/pi-mono). Connects to Model Context Protocol servers and exposes their tools to the agent.
...
```bash
pi install git:github.com/Denveous/pi-mcp
```

Or manually: clone this repo, run `npm install`, and load with `pi -e /path/to/pi-mcp`.
